window.wxMiniGame = function (exports, Laya) {
	...
	class MiniFileMgr {
			...
			static onSetVersion(preBasePath, nowBasePath)
			{
				...
				if(manifest)
				{
					for(var pathKey in manifest)
					{
						var pathVer = manifest[pathKey];
						var preUrl = preBasePath + pathVer;
						var fileObj = this.getFileInfo(preUrl);
						if(fileObj)
						{
							var nowUrl = nowBasePath + pathVer;
							fileObj.readyUrl = nowUrl;
							delete MiniFileMgr.fakeObj[preUrl];
							MiniFileMgr.fakeObj[nowUrl] = fileObj;
						}
					}
					MiniFileMgr.writeFilesList("", JSON.stringify(MiniFileMgr.filesListObj), true);
				}
				console.log("替换后",MiniFileMgr.fakeObj)
			}
			...
			static getWxFileMap()
			{
				if(!this.wxFileMap)
				{
					this.wxFileMap = new Map();
					var list = MiniFileMgr.fs.readdirSync(MiniFileMgr.fileNativeDir);
					for(let path of list)
					{
						this.wxFileMap.set(path, true);
					}
				}
				return this.wxFileMap;
			}

			static onFileAdd(path)
			{
				path = path.replace(MiniFileMgr.fileNativeDir + "/", "");
				// console.log("新增文件:", path);
				let map = this.getWxFileMap();
				map.set(path, true);
			}

			static onFileRemove(path)
			{
				path = path.replace(MiniFileMgr.fileNativeDir + "/", "");
			...
			static read(filePath, encoding = "utf8", callBack = null, readyUrl = "", isSaveFile = false, fileType = "") {
					...
					// console.log(`>>>微信读取文件 read,  filePath=${filePath}, fileUrl=${fileUrl}`);
					...
					// console.log(`>>>微信读取文件失败 read,  filePath=${filePath}, fileUrl=${fileUrl}`);
									if (data && readyUrl != "")
			...
			static downFiles(fileUrl, encoding = "utf8", callBack = null, readyUrl = "", isSaveFile = false, fileType = "", isAutoClear = true) {
					// console.log(`>>>微信下载文件 downFiles, isSaveFile=${isSaveFile}, fileType=${fileType}, isAutoClear=${isAutoClear}, fileUrl=${fileUrl}, readyUrl=${readyUrl}`);
					var downloadTask = MiniFileMgr.wxdown({
							url: fileUrl,
							success: function (data) {
									// console.log(`>>>微信下载文件成功 downFiles, data.statusCode=${data.statusCode}， isSaveFile=${isSaveFile}, fileType=${fileType}, isAutoClear=${isAutoClear}, fileUrl=${fileUrl}, readyUrl=${readyUrl}`);
			...
			static readFile(filePath1, encoding = "utf8", callBack = null, readyUrl = "", isSaveFile = false, fileType = "", isAutoClear = true) {

					var filePath = Laya.URL.getAdptedFilePath(filePath1);

					// console.log(`>>>微信读取文件 readFile,   isSaveFile=${isSaveFile},fileType=${fileType}, filePath1=${filePath1}, filePath=${filePath}, readyUrl=${readyUrl}`);
					MiniFileMgr.fs.readFile({
							filePath: filePath,
							encoding: encoding,
							success: function (data) {
											// console.log(`>>>微信读取文件成功 readFile  1,   isSaveFile=${isSaveFile},fileType=${fileType}, filePath1=${filePath1}, filePath=${filePath}, readyUrl=${readyUrl}`);

											if (readyUrl.indexOf("http://") != -1 || readyUrl.indexOf("https://") != -1) {

													// console.log(`>>>微信读取文件成功 readFile  2`);

													if (MiniAdpter.autoCacheFile || isSaveFile) {
															// console.log(`>>>微信读取文件成功 readFile  3`);
															callBack != null && callBack.runWith([0, data]);
															MiniFileMgr.copyFile(filePath, readyUrl, null, encoding, isAutoClear);
													} else {
															// console.log(`>>>微信读取文件成功 readFile  4`);
															callBack != null && callBack.runWith([0, data]);
													}
											} else {
													// console.log(`>>>微信读取文件成功 readFile  5`);
													callBack != null && callBack.runWith([0, data]);
											}
									},

									fail: function (data) {
											// console.log(`>>>微信读取文件失败 readFile readyUrl=${readyUrl}, filePath=${filePath}`);
											if (data)
													callBack != null && callBack.runWith([1, data]);
									}
			...
			static downOtherFiles(fileUrl, callBack = null, readyUrl = "", isSaveFile = false, isAutoClear = true) {
					// console.log(`>>>微信下载文件 downOtherFiles, isSaveFile=${isSaveFile},  isAutoClear=${isAutoClear}, fileUrl=${fileUrl}, readyUrl=${readyUrl}`);
			...
			static downLoadFile(fileUrl, fileType = "", callBack = null, encoding = "utf8") {
					// console.log(`>>>MiniFileMgr downLoadFile fileUrl=${fileUrl}, fileType=${fileType}`);
			...
			static copyFile(tempFilePath, readyUrl, callBack, encoding = "", isAutoClear = true) {
					...
					// console.log(`>>>set fakeObj  fileurlkey=${fileurlkey} `, MiniFileMgr.fakeObj[fileurlkey]);
					var totalSize = 50 * 1024 * 1024;
					var chaSize = 4 * 1024 * 1024;
					var fileUseSize = MiniFileMgr.getCacheUseSize();
					if (fileObj) {
							if (fileObj.readyUrl != readyUrl || fileObj.size == 0) {
									MiniFileMgr.fs.getFileInfo({
											...
											success: function (data) {
															if ((isAutoClear && (fileUseSize + chaSize + data.size) >= totalSize)) 
															{
																	...
																	// console.log(`>>> 需要清理缓存1 data.size=${bToStr(data.size)}, fileUseSize=${bToStr(fileUseSize)}, chaSize=${bToStr(chaSize)}, now=${bToStr(fileUseSize + chaSize + data.size)}, totalSize=${bToStr(totalSize)}`);
														
																	MiniFileMgr.onClearCacheRes();
															}
															console.log(`>>> 删除之前版本的文件 data.size=${bToStr(data.size)}, fileUseSize=${bToStr(fileUseSize)}, chaSize=${bToStr(chaSize)}, now=${bToStr(fileUseSize + chaSize + data.size)}, totalSize=${bToStr(totalSize)}`);
														
															MiniFileMgr.deleteFile(tempFilePath, readyUrl, callBack, encoding, data.size);
													},
													fail: function (data) {
														console.log(`>>> 读取文件信息失败 tempFilePath=${tempFilePath}`);
							...
							{
								console.log(`>>> 直接回调 tempFilePath=${tempFilePath}`);
							...
							MiniFileMgr.fs.getFileInfo({
									...
									success: function (data) {
													...
													if ((isAutoClear && (fileUseSize + chaSize + data.size) >= totalSize)) 
													{
															if (data.size > MiniAdpter.minClearSize)
																	MiniAdpter.minClearSize = data.size;
															// console.log(`>>> 需要清理缓存2 data.size=${bToStr(data.size)}, fileUseSize=${bToStr(fileUseSize)}, chaSize=${bToStr(chaSize)}, now=${bToStr(fileUseSize + chaSize + data.size)}, totalSize=${bToStr(totalSize)}`);
															MiniFileMgr.onClearCacheRes();
													}
													// console.log(`>>>微信拷贝文件 copyFile, readyUrl=${readyUrl}, tempFileName=${tempFileName}, saveFilePath=${saveFilePath} `);

													MiniFileMgr.fs.copyFile({
															srcPath: tempFilePath,
															destPath: saveFilePath,
															success: function (data2) {
																	MiniFileMgr.onFileAdd(saveFilePath);
																	MiniFileMgr.onSaveFile(readyUrl, tempFileName, true, encoding, callBack, data.size);
															}, fail: function (data) {
																// console.log(`>>>微信拷贝文件失败 copyFile, readyUrl=${readyUrl}, tempFileName=${tempFileName}, saveFilePath=${saveFilePath} `);
			...
			static onClearCacheRes() {
					...
					// console.log(`>>>onClearCacheRes: MiniFileMgr.getCacheUseSize=${bToStr(MiniFileMgr.getCacheUseSize())}， minClearSize=${bToStr(memSize)}`);
					...
					for (var i = 1, sz = tempFileListArr.length; i < sz; i++) {
							...
							// console.log(`>>>onClearCacheRes: MiniFileMgr.getCacheUseSize=${bToStr(MiniFileMgr.getCacheUseSize())}， minClearSize=${bToStr(memSize)}, clearSize=${bToStr(clearSize)}, fileObj.size=${bToStr(fileObj.size)}, fileObj.readyUrl=${fileObj.readyUrl}`);
			...
			static deleteFile(tempFileName, readyUrl = "", callBack = null, encoding = "", fileSize = 0) {
					...
					console.log(`>>>微信删除文件 deleteFile, readyUrl=${readyUrl}, deleteFileUrl=${deleteFileUrl} `);
					MiniFileMgr.onFileRemove(deleteFileUrl);
					MiniFileMgr.fs.unlink({
							filePath: deleteFileUrl,
							success: function (data) {
									if (tempFileName != "") {
											var saveFilePath = MiniFileMgr.getFileNativePath(tempFileName);
											console.log(`>>>微信拷贝文件 deleteFile, readyUrl=${readyUrl}, tempFileName=${tempFileName}, saveFilePath=${saveFilePath} `);
											MiniFileMgr.fs.copyFile({
			...
			static deleteAll() {
					...
					{
						var deleteFileUrl = MiniFileMgr.fileNativeDir + "/" + path;
						var e = MiniFileMgr.fs.unlinkSync(deleteFileUrl);
					}
					
					MiniFileMgr.onFileClearAll();
			...
			static onSaveFile(readyUrl, md5Name, isAdd = true, encoding = "", callBack = null, fileSize = 0) {
				var fileurlkey = readyUrl;
					...
					if (isAdd) {
							...
							if(fileSize == 0)
							{
								console.error(`缓存的文件大小为0 不正常 readyUrl=${readyUrl}, md5Name=${md5Name}`);
							}


							if(md5Name.indexOf('wxfile://') != -1)
							{
								console.error(`缓存的文件md5Name携带了wxfile://  readyUrl=${readyUrl}, md5Name=${md5Name}`);
								// md5Name = md5Name.replace('wxfile://', '');
							}
					...
					// console.log(`>>>onSaveFile isAdd=${isAdd}, fileSize=${bToStr(fileSize)}, fileUsedSize=${bToStr(MiniFileMgr.filesListObj['fileUsedSize'])} readyUrl=${readyUrl}, md5Name=${md5Name}`);
					
			}
			static writeFilesList(fileurlkey, filesListStr, isAdd) {
					var listFilesPath = MiniFileMgr.fileNativeDir + "/" + MiniFileMgr.fileListName;

					// console.log(`>>>微信写入文件 writeFilesList, isAdd=${isAdd} fileurlkey=${fileurlkey}, listFilesPath=${listFilesPath}, filesListStr=${filesListStr} `);
			...
			static existDir(dirPath, callBack) {
					// console.log(`>>>微信创建文件夹 existDir, dirPath=${dirPath}`);
			...
			static readSync(filePath, encoding = "utf8", callBack = null, readyUrl = "") {
					...
					var filesListStr;
					// console.log(`>>>微信读取文件 readSync,  filePath=${filePath}, fileUrl=${fileUrl}`);
					try {
	...
	class MiniSound extends Laya.EventDispatcher {
			...
			load(url) {
				console.log("MiniSound.load", url);
	...
	class MiniLoader extends Laya.EventDispatcher {
			...
			_loadResourceFilter(type, url, relativePath) {
					...
					var userUrl = MiniAdpter.getNativeUserUrl(url1);
					// console.log(">>>userUrl " + userUrl);
					// userUrl = null;
					
					var fileObj = MiniFileMgr.getFileInfo(url1);
					...
					// console.log(`>>>_loadResourceFilter 3, type=${type}, url=${url}, url1=${url1}, userUrl=${userUrl}`);
							...
							// {
								...
								if (userUrl) 
								{
									thisLoader._loadImage(userUrl);
								} 
								else 
								{
									MiniLoader._transformImgUrl(url1, type, thisLoader);
								}
							// }
			...
			_loadHttpRequestWhat(url, contentType) {
					...
					var isVersionJson = url.indexOf("version.json") != -1;
					// console.log(`>>>MiniLoader_loadHttpRequestWhat url=${url}, contentType=${contentType}`);
					var thisLoader = this;
					...
					if (Laya.Loader.preLoadedMap[url])
							...
							// console.log(`>>>MiniLoader_loadHttpRequestWhat 1 url=${url}, contentType=${contentType}, tempurl=${tempurl}, MiniAdpter.AutoCacheDownFile=${MiniAdpter.AutoCacheDownFile}`);
							if (isVersionJson && url.indexOf(MiniAdpter.window.wx.env.USER_DATA_PATH) == -1 && (tempurl.indexOf("http://") != -1 || tempurl.indexOf("https://") != -1) && !MiniAdpter.AutoCacheDownFile) {

									// console.log(`>>>MiniLoader_loadHttpRequestWhat 2 url=${url}, contentType=${contentType}, tempurl=${tempurl}`);
									...
									// console.log(`>>>MiniLoader_loadHttpRequestWhat 3 url=${url}, contentType=${contentType}, fileObj=${fileObj}`);
									if (fileObj) {
											fileObj.encoding = fileObj.encoding == null ? "utf8" : fileObj.encoding;
											var userUrl = MiniAdpter.getNativeUserUrl(url);
											// console.log(`>>>MiniLoader_loadHttpRequestWhat 3--1 url=${url}, contentType=${contentType}, fileObj.url=${fileObj.url}, userUrl=${userUrl}`);
											MiniFileMgr.readFile(userUrl, encoding, new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [url, contentType, thisLoader]), url);
									} else if (thisLoader.type == "image" || thisLoader.type == "htmlimage") {

											// console.log(`>>>MiniLoader_loadHttpRequestWhat 4 url=${url}, contentType=${contentType}, fileObj=${fileObj}`);
											...
											if (contentType != Laya.Loader.IMAGE && ((tempurl.indexOf("http://") == -1 && tempurl.indexOf("https://") == -1) || MiniFileMgr.isLocalNativeFile(url))) {
													...
													// console.log(`>>>MiniLoader_loadHttpRequestWhat 5 url=${url}, contentType=${contentType}, fileObj=${fileObj}`);
													...
													// console.log(`>>>MiniLoader_loadHttpRequestWhat 6 url=${url}, contentType=${contentType}, fileObj=${fileObj}`);
			...
			static _transformImgUrl(url, type, thisLoader) {
					...
					// console.log(`>>>_transformImgUrl , type=${type}, url=${url}, userUrl=${userUrl},  Laya.URL.formatURL(url)=${Laya.URL.formatURL(url)}`);
					if (!userUrl) {
							var tempUrl = Laya.URL.formatURL(url);
	...
	class MiniAdpter {
			...
			static downLoadFile(fileUrl, fileType = "", callBack = null, encoding = "utf8") {
					// console.log(`>>>MiniAdpter downLoadFile fileUrl=${fileUrl}, fileType=${fileType}, MiniFileMgr.getFileInfo(fileUrl)=${MiniFileMgr.getFileInfo(fileUrl)}`);
			...
			static getNativeUserUrl(fileUrl) {
					if (!fileUrl) return null;
					if (fileUrl.indexOf('https:') == -1 || fileUrl.indexOf('http:') == -1) {
							if (fileUrl.indexOf(':') == -1) {
									fileUrl = Laya.URL.formatURL(fileUrl);
							}
					}



					let info = this.getFileInfo(fileUrl);
					if (info) 
					{
						if(!MiniFileMgr.hasFile(info.md5))
						{
							console.warn("检测实际不存在文件:", info.md5);
							MiniFileMgr.onSaveFile(info.readyUrl, info.md5, false);
							return null;
						}

						if(info.md5.indexOf('wxfile://') != -1)
						{
							return info.md5;
						}

						return Laya.MiniFileMgr.fileNativeDir + "/" + info.md5;
					}
					return null;
			}
	...
	class MiniImage {
			_loadImage(url) {
					// console.log(">>>MiniImage._loadImage ", url);
					var thisLoader = this;
					if (MiniAdpter.isZiYu) {
